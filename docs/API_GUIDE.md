...
| **Output.artifact** | `GET /v1/tasks/:id` 的 `task.output.artifact` | 產出物 — 真正交付的東西 | typed discriminated union，每個 kind 一個 renderer |

**核心原則：**
- `messages` 不再夾帶 HTML / JSON 大段預覽 — 那是 artifact 的工作
- `artifact.kind` 是前端 dispatch 的 key — 每種 kind 寫一個 renderer 元件
- 同一個 task 在不同階段會有不同的 artifact（eeat-questions → blog-article → 加上 published 蓋章）
...
type Artifact =
  ...
  | { kind: 'seo-plan';        data: SeoPlanData }       // 由 seo-strategist 產
  | { kind: 'product-plan';    data: ProductPlanData }   // 由 product-planner 產
  | { kind: 'eeat-questions';  data: { summary?, questions, askedAt } }  // 由 shopify-blog-writer Stage 1 產
  | { kind: 'clarification';   data: { question } };     // 由 supervisor 產

interface BlogArticleData {
  title: string;
  bodyHtml: string;       // 真正的 HTML — DOMPurify + iframe srcdoc 渲染
  summaryHtml: string;    // Shopify 用的 meta 描述（HTML, ≤200 字）
...
interface ProductContentData {
  ...
  productType?: string;
  language: string;
  imageUrls: string[];    // CF R2 public URLs
}
...
function ArtifactPanel({ artifact }: { artifact: Artifact }) {
  switch (artifact.kind) {
    case 'blog-article':
      return <BlogArticleView data={artifact.data} published={artifact.published} />;
    case 'product-content':
      return <ProductContentView data={artifact.data} published={artifact.published} />;
    case 'seo-plan':
      return <SeoPlanView data={artifact.data} />;
    case 'product-plan':
      return <ProductPlanView data={artifact.data} />;
    case 'eeat-questions':
      return <EeatQuestionsView data={artifact.data} />;
    case 'clarification':
      return <ClarificationView data={artifact.data} />;
  }
}

// 渲染 article — bodyHtml 是真正 HTML（不是 markdown），summary 是 markdown
function BlogArticleView({ data, published }) {
  const safeHtml = DOMPurify.sanitize(data.bodyHtml);
  return (
    <article>
      <h1>{data.title}</h1>
      {/* 上半：給老闆看的匯報（zh-TW Markdown） */}
      {data.summary && <ReactMarkdown>{data.summary}</ReactMarkdown>}
      {/* 下半：實際文章內容（HTML） */}
      <iframe srcDoc={safeHtml} sandbox="allow-same-origin" />
      {published && <a href={published.articleUrl}>已發布到 Shopify ✓</a>}
    </article>
  );
}
```
...
| `waiting` (eeat-questions) | progressNote: 「有兩個 EEAT 問題想請老闆回答」 | EEAT 問題清單 + 回答框 |
| `waiting` (blog-article) | progressNote: 「草稿好了，老闆看一下開頭那段」 | 文章預覽 |
| `done` (published 已蓋) | progressNote: 「已發布到 Shopify ✓」 | 文章預覽 + 「去後台看」連結 |
...
| `output.artifact` | 任務當前階段的 typed deliverable（discriminated union，見「§5.1 三個介面」） |
...
| `agent.questions.asked` | Blog Writer Stage 1：EEAT 問題產生（artifact `eeat-questions`） |
| `agent.draft.ready` | Blog Writer Stage 2：草稿完成（artifact `blog-article`） |
| `agent.plan.ready` | SEO Strategist (artifact `seo-plan`) / Product Planner (artifact `product-plan`) 計畫完成 |
| `agent.content.ready` | Product Designer 文案 + 圖片完成（artifact `product-content`） |
...
| `tool.completed` | tool 成功，結果蓋章在 `task.output.artifact.published` |
| `tool.failed` | tool 失敗，task 轉 failed |

**`agent.*.ready` 事件的 `data` 欄位會帶 `artifactKind`**，前端可以直接用來決定點擊後要打開哪個 artifact panel：
```js
// 收到 SSE 事件
{ event: 'agent.draft.ready', data: { artifactKind: 'blog-article', title: '夏日穿搭...' }, ... }
```
...
### Stage 1 output 形狀（eeat-questions artifact）
...
```json
{
  ...
  "output": {
    ...
    "eeatPending": { /* 同上 questions+askedAt — 旗標用，runner 用來判斷 Stage 2 該不該觸發 */ }
  }
}
```

UI 應該渲染：
- artifact panel：問題清單，`optional: true` 標示「可選」
...
```json
{
  ...
  "output": {
    ...
    "pendingToolCall": {
      ...
      "args": { /* same as artifact.data + author */ }
    }
  }
}
```
...
- artifact panel：dispatch on `artifact.kind === 'blog-article'` → 上半渲染 `data.summary`（zh-TW Markdown 匯報，用 marked / react-markdown），下半渲染 `data.bodyHtml`（DOMPurify + iframe srcdoc）
- 中間：「按 Approve 後會發到 Shopify 部落格 `<blogHandle 或第一個 blog>`，狀態 `<draft 或 published>`」
- CTA：[Approve & Publish] (`finalize:true`) / [Feedback]

#### `finalize:true` 之後

`task.output.artifact` 上多一個 `published` 蓋章，artifact `kind` 不變：
```json
{
  "artifact": {
    "kind": "blog-article",
...
UI 拿到 200 後可立刻顯示「已草稿到 Shopify，[去後台看](artifact.published.articleUrl)」。
若 `published.status='published'` 表示已對讀者公開。
...
```json
{
  ...
  "output": {
    ...
    "pendingToolCall": {
      ...
      "args": { /* same as artifact.data + images: [{url}] */ }
    }
  }
}
```

`finalize:true` approve 後 → `artifact.published` 蓋章：
```json
"artifact": {
  "kind": "product-content",
  "data": { /* 同上 */ },
...
/**
 * Artifact — typed deliverable on task.output.artifact. UI dispatches on `kind`.
 * See §5.1 for the full contract.
 */
type Artifact =
  | { kind: 'blog-article';    data: BlogArticleData;    published?: BlogPublishedMeta }
  | { kind: 'product-content'; data: ProductContentData; published?: ProductPublishedMeta }
  ...
  | { kind: 'eeat-questions';  data: { summary?: string; questions: { question: string; hint?: string; optional?: boolean }[]; askedAt: string } }
  | { kind: 'clarification';   data: { question: string } };

interface BlogArticleData {
  title: string;
  bodyHtml: string;
  summaryHtml: string;     // Shopify meta excerpt (HTML)
...
interface ProductPlanVariant {
  ...
  imagePlan: { purpose: string; styleHint: string; priority: 'required' | 'optional' }[];
  assignedAgent: 'product-designer';
  scheduledAt?: string;
}
