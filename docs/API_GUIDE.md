...
type Artifact =
  ...
  | { kind: 'eeat-questions';  data: { questions, askedAt } }  // 由 shopify-blog-writer Stage 1 產
  | { kind: 'clarification';   data: { question } };     // 由 supervisor 產

interface BlogArticleData {
  title: string;
  bodyHtml: string;       // ← 這是真正的 HTML，前端可用 DOMPurify + iframe 渲染
  summaryHtml: string;
  tags: string[];
  language: string;
  author?: string;
...
- artifact panel：dispatch on `artifact.kind === 'blog-article'` → 渲染標題 / 摘要 / `bodyHtml`（DOMPurify + iframe）
...
type Artifact =
  ...
  | { kind: 'eeat-questions';  data: { questions: { question: string; hint?: string; optional?: boolean }[]; askedAt: string } }
